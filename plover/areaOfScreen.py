...
def lookup(strokes):
    if strokes[0] == "":
        raise KeyError  # skip half the strokes early
    x = 50
    y = 50
    size = 100
    for stroke in strokes:
        if not stroke.startswith(uniqueStarter):
            raise KeyError  # ignore non area of screen commands
        position = stroke[len(uniqueStarter) :]  # remove the uniqueStarter
        if position == "":
            break  # if no selection is made, use the current one
    ...
    # return the plover command, with added "%" symbols
    return "{PLOVER:mouse_set:" + str(int(x)) + "%," + str(int(y)) + "%}"
